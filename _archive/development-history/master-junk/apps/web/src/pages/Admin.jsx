...
import { Globe, Mail, ShieldCheck, LayoutDashboard, Users, CreditCard, Settings, LogOut, Sun, Moon, Activity, Key, Smartphone, HardDrive, DownloadCloud, Server, Cpu, Database, Network, Trash2, Box, X, Zap, Edit2, Eye, EyeOff } from 'lucide-react';
...
export default function Admin() {
  ...
  const handleSaveSmtp = async () => {
    ...
    try {
      const token = localStorage.getItem('hidewin_token');
      const res = await fetch('http://localhost:8000/api/admin/smtp', {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  const handleTestSmtp = async () => {
    ...
    try {
      const token = localStorage.getItem('hidewin_token');
      const res = await fetch('http://localhost:8000/api/admin/smtp/test', {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
  ...
  const submitModal = async () => {
    ...
    try {
      if (type === 'ADD_USER') {
        ...
        const res = await fetch(`http://localhost:8000/api/admin/user/create?role=${modalForm.role}`, {
          method: 'POST',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
        ...
        const res = await fetch(`http://localhost:8000/api/admin/user/${payload.id}`, {
          method: 'DELETE',
          headers: { 'Authorization': `Bearer ${token}` }
        });
        await handleRes(res);
        ...
        const res = await fetch(`http://localhost:8000/api/admin/user/${payload.id}/role`, {
          method: 'PUT',
          headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
          ...
          <div className="tab-content fade-in settings-layout">
            ...
            <div className="settings-content">
                  ...
                  <div className="fade-in">
                    ...
                    <div className="card">
                      ...
                      <div className="input-group">
                        ...
                        <input type="text" className="input-field" placeholder="8000" value={appSettings?.dns_port || ''} onChange={(e) => handleSettingChange('dns_port', e.target.value)} />
                      </div>
              ...
              {activeSettingsTab === 'architecture' && (
                <div className="fade-in">
                  <div style={{ marginBottom: '24px' }}>
                    <h2 style={{ fontSize: '24px', fontWeight: 700, marginBottom: '8px' }}>Advanced Architecture & Load Balancing</h2>
                    <p style={{ color: 'var(--text-muted)' }}>Configure high-throughput messaging pipelines to handle thousands of concurrent user sessions with zero latency.</p>
                  </div>
                  ...
                  <div className="card">
                    <div style={{ display: 'flex', gap: '16px', alignItems: 'center', marginBottom: '24px' }}>
                      ...
                      <div>
                        <h2 style={{ fontSize: '18px', fontWeight: 600 }}>Kafka Event-Driven Architecture</h2>
                        <p style={{ color: 'var(--text-muted)', fontSize: '14px' }}>Stream session events via Apache Kafka to distribute load.</p>
                      </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px' }}>
                      <div className="input-group">
                        <label className="input-label">Kafka Broker URLs</label>
                        <input type="text" className="input-field" placeholder="e.g., broker1:9092, broker2:9092" disabled />
                      </div>
                      <div className="input-group">
                        <label className="input-label">Session Topic Name</label>
                        <input type="text" className="input-field" placeholder="hidewin_sessions_topic" disabled />
                      </div>
                      <div className="input-group">
                        <label className="input-label">Consumer Group ID</label>
                        <input type="text" className="input-field" placeholder="hidewin-ai-workers" disabled />
                      </div>
                      <div className="input-group">
                        <label className="input-label">Replication Factor</label>
                        <input type="number" className="input-field" placeholder="3" disabled />
                      </div>
                    </div>
                    <div style={{ marginTop: '16px', padding: '16px', background: 'var(--bg-subtle)', borderRadius: '8px', border: '1px solid var(--border)', fontSize: '14px', color: 'var(--text-muted)' }}>
                      <strong>Status:</strong> Waiting for backend Kafka microservices to be deployed. Once deployed, enter broker credentials here to enable sub-millisecond event streaming.
                    </div>
                  </div>
                </div>
              )}
