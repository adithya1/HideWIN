...
# 1. Remove the window controls from the notes-toolbar (that I just added)
wc_regex = r'<!-- OS Window Controls -->.*?</div>'
text = re.sub(wc_regex, '', text, flags=re.DOTALL)

# 2. Inside renderTabsContainer, we add the controls to the right side of the tabs-bar.
# Let's find where tabs-bar ends. 
# `<button class="hdr-icon-btn tab-add" ...>+</button>`
# `</div>`  <- end of tabs-list
tab_add_regex = r'(<button class="hdr-icon-btn tab-add" title="New Note" @click=\$\{.*?\}\)>\+</button>\s*</div>)'

note_controls = r'''\g<1>
                        <!-- Note-level Window Controls -->
                        <div class="window-controls" style="display: flex; gap: 4px; padding-right: 8px; align-items: center;">
