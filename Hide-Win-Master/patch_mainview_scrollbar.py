...
posh_scrollbar_css = """
        /* GLOBAL POSH LEAN SCROLLBAR FOR THIS VIEW */
        ::-webkit-scrollbar {
...
if "GLOBAL POSH LEAN SCROLLBAR FOR THIS VIEW" not in code:
    code = code.replace("static styles = css`", f"static styles = css`{posh_scrollbar_css}")
    with open('src/components/views/MainView.js', 'w', encoding='utf-8') as f:
        f.write(code)
    print("Injected posh scrollbar into MainView.js")
else:
