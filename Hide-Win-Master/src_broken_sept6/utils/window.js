...
const DEFAULT_SESSION_WINDOW_SIZE = { width: 650, height: 450 };
...
function updateGlobalShortcuts(keybinds, mainWindow, sendToRenderer, geminiSessionRef) {
            ...
            globalShortcut.register(keybinds.setMouseDetectable, () => {
                if (mouseEventsIgnored) {
                    ...
                    if (typeof stealthActive !== 'undefined' && stealthActive) {
                        stealthActive = false;
                        stopStealthMode();
                        mainWindow.webContents.send('set-stealth-state', stealthActive);
                    }
        ...
        try {
            ...
            blockerProcess.stdout.on('data', (chunk) => {
                ...
                for (const raw of lines) {
                    ...
                    const dotX = (frozenScreenX + redDotDX) - bounds.x;
                    const dotY = (frozenScreenY + redDotDY) - bounds.y;

                    if (line.startsWith('MOVE:')) {
                        // Send native mouseMove to Chromium
                        mainWindow.webContents.sendInputEvent({ type: 'mouseMove', x: dotX, y: dotY });
                        // Also notify renderer to draw the red dot
                        mainWindow.webContents.send('move-stealth-cursor', { x: dotX, y: dotY });
                    } else if (line === 'LDOWN') {
                        if (mainWindow && !mainWindow.isFocused()) {
                            mainWindow.focus();
                        }
                        mainWindow.webContents.sendInputEvent({ type: 'mouseDown', button: 'left', clickCount: 1, x: dotX, y: dotY });
                    } else if (line === 'LUP') {
                        mainWindow.webContents.sendInputEvent({ type: 'mouseUp', button: 'left', clickCount: 1, x: dotX, y: dotY });
                    } else if (line === 'RDOWN') {
                        mainWindow.webContents.sendInputEvent({ type: 'mouseDown', button: 'right', clickCount: 1, x: dotX, y: dotY });
                    } else if (line === 'RUP') {
                        mainWindow.webContents.sendInputEvent({ type: 'mouseUp', button: 'right', clickCount: 1, x: dotX, y: dotY });
                    } else if (line.startsWith('WHEEL:')) {
                        const delta = parseInt(line.slice(6), 10) || 0;
                        mainWindow.webContents.sendInputEvent({ type: 'mouseWheel', deltaY: delta, x: dotX, y: dotY });
                    }
                }
            });

            blockerProcess.stderr.on('data', (d) => console.error('MouseBlocker stderr:', d.toString()));
            ...
            setTimeout(() => {
                if (!mainWindow || mainWindow.isDestroyed()) return;
                const bounds = mainWindow.getBounds();
                const initX = frozenScreenX - bounds.x;
                const initY = frozenScreenY - bounds.y;
                mainWindow.webContents.sendInputEvent({ type: 'mouseMove', x: initX, y: initY });
                mainWindow.webContents.send('move-stealth-cursor', { x: initX, y: initY });
            }, 50);

            console.log('Stealth ON — cursor physically frozen at', frozenScreenX, frozenScreenY);
        } catch (err) {
            console.error('Failed to start MouseBlocker.exe:', err.message);
            blockerProcess = null;
        }
    };
    ...
    const stopStealthMode = () => {
        ...
        console.log('Stealth OFF — cursor released from', frozenScreenX, frozenScreenY);
    };
    ...
    if (keybinds.toggleMouseVisibility) {
        try {
            globalShortcut.register(keybinds.toggleMouseVisibility, () => {
                ...
                // Send toggle to renderer FIRST so cursor-hidden class is applied
                // before startStealthMode sends the initial red-dot position.
                mainWindow.webContents.send('set-stealth-state', stealthActive);
                ...
                console.log(`Stealth mode: ${stealthActive ? 'ON' : 'OFF'}`);
            });
            console.log(`Registered toggleMouseVisibility: ${keybinds.toggleMouseVisibility}`);
        } catch (error) {
            console.error(`Failed to register toggleMouseVisibility (${keybinds.toggleMouseVisibility}):`, error);
        }
    }
