const { app, BrowserWindow, desktopCapturer, session, shell, ipcMain, dialog, Menu } = require('electron');
...
function setupGeneralIpcHandlers() {
    ...
    ipcMain.handle('pin-to-desktop', async (event, noteId, noteTitle) => {
        try {
            ...
            // Electron's shell.writeShortcutLink sometimes fails silently with OneDrive desktops.
            ...
            return new Promise((resolve) => {
                exec(`powershell -NoProfile -NonInteractive -WindowStyle Hidden -Command "${psCommand}"`, (error) => {
                    if (error) {
