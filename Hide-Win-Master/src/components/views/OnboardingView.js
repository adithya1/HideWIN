...
export class OnboardingView extends LitElement {
    ...
    async completeOnboarding() {
        if (this.contextText.trim()) {
            await hideWin.storage.updatePreference('customPrompt', this.contextText.trim());
        }
        await hideWin.storage.updateConfig('onboarded', true);
        this.onComplete();
    }
    ...
    renderSlide() {
        if (this.currentSlide === 0) {
            return html`
                <div class="slide">
                    <div class="slide-title">HideWin</div>
