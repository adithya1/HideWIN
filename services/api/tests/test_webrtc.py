...
def test_third_eye_agent_response(client, db_session):
    """Test the 'Third-Eye' WebRTC query intercept natively."""
    
    # 1. Setup authorized user
    user = User(email="webrtc@test.com", role="USER", is_verified=True)
    db_session.add(user)
    db_session.commit()
    db_session.refresh(user)
    
    token = create_access_token(data={"sub": "webrtc@test.com", "id": user.id, "role": "USER"})
