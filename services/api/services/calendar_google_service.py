...
GOOGLE_REDIRECT_URI = "http://localhost:5173/settings/calendar/google/callback"
    ...
    async def update_event(user: User, meeting: Meeting, db: AsyncSession) -> bool:
        ...
        async with httpx.AsyncClient() as client:
            resp = await client.put(
                f"https://www.googleapis.com/calendar/v3/calendars/primary/events/{meeting.google_event_id}",
                headers=headers,
                json=event_payload
            )
            if resp.status_code == 200:
