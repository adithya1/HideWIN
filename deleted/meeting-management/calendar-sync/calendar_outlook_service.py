...
OUTLOOK_REDIRECT_URI = f"{settings.WEB_BASE_URL.rstrip('/')}/settings/calendar/outlook/callback"


class OutlookCalendarService:
    @staticmethod
    def get_auth_url(state: str) -> str:
        base_url = settings.OUTLOOK_OAUTH_AUTHORIZE_URL
        params = {
    ...
    async def exchange_code(code: str) -> dict:
        token_url = settings.OUTLOOK_OAUTH_TOKEN_URL
        data = {
            "client_id": get_outlook_client_id(),
            "scope": "offline_access Calendars.ReadWrite",
            "code": code,
    ...
    async def create_event(user: User, meeting: Meeting, db: AsyncSession) -> str:
        ...
        async with httpx.AsyncClient() as client:
            resp = await client.post(
                settings.OUTLOOK_CALENDAR_EVENTS_URL,
                headers=headers,
                json=event_payload
            )
    ...
    async def update_event(user: User, meeting: Meeting, db: AsyncSession) -> bool:
        ...
        async with httpx.AsyncClient() as client:
            resp = await client.patch(
                f"{settings.OUTLOOK_CALENDAR_EVENTS_URL}/{urllib.parse.quote(meeting.outlook_event_id, safe='')}",
                headers=headers,
                json=event_payload
            )
            if resp.status_code == 200:
