...
# Remove the old stealth variables
content = re.sub(r'let stealthActive = false;\s*let stealthPaused = false;\s*let blockerProcess = null;\s*let frozenScreenX = 0;\s*let frozenScreenY = 0;\s*let redDotDX = 0;\s*let redDotDY = 0;\s*let blockerExePath = null;', 'let stealthActive = false;\nlet stealthPaused = false;\nlet frozenScreenX = 0;\nlet frozenScreenY = 0;\nlet isMomentaryStealth = false;', content)

# Remove the mouse blocker Python comment and extraction logic
content = re.sub(r'//  5\. Hide the OS cursor via Python.*?const \{ ipcMain \} = require\(\'electron\'\);', "const { ipcMain } = require('electron');", content, flags=re.DOTALL)

# Replace startStealthMode
...
with open(path, 'w', encoding='utf-8') as f:
    f.write(content)
print("Applied clean Pointer Lock patch!")
