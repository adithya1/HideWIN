...
old_security_btn = """                            <button class="drawer-child-btn ${(this.currentView === 'customize' && this.currentSettingsTab === 'security') ? 'active' : ''}" @click=${() => { this.currentSettingsTab = 'security'; this.navigate('customize'); this.isMobileMenuOpen = false; }}>
...
new_btns = """                            <button class="drawer-child-btn ${(this.currentView === 'customize' && this.currentSettingsTab === 'security') ? 'active' : ''}" @click=${() => { this.currentSettingsTab = 'security'; this.navigate('customize'); this.isMobileMenuOpen = false; }}>
                                <div class="drawer-child-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 10 0v4"></path></svg></div>
                                Security
                            </button>
...
code = code.replace(old_security_btn, new_btns)
